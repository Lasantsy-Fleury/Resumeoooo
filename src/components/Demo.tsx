...
export default function Demo() {
  ...
  return (
    <section id="demo" className="py-24 sm:py-32 bg-surface-card">
      <div className="max-w-6xl mx-auto px-6 sm:px-8">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="relative rounded-2xl bg-white border border-surface-divider overflow-hidden" style={{ boxShadow: '0 24px 64px rgba(0,0,0,0.1), 0 8px 24px rgba(0,0,0,0.05)' }}>
            {/* Window chrome */}
            ...
            {!isSummarized ? (
              ...
              <div className="p-12 sm:p-16">
                <label className="flex flex-col items-center gap-5 cursor-pointer group">
                  <div className="w-20 h-20 rounded-2xl bg-accent/[0.05] border-2 border-dashed border-accent/25 flex items-center justify-center group-hover:bg-accent/[0.1] group-hover:border-accent/50 transition-all duration-300 scale-100 group-hover:scale-105">
                    <svg className="w-9 h-9 text-accent/50 group-hover:text-accent/70 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <div className="flex gap-2">
                    {['PDF', 'TXT', 'URL'].map((fmt) => (
                      <span key={fmt} className="px-3.5 py-1.5 rounded-full bg-surface-card text-xs font-medium text-ink-muted border border-surface-divider">
                        {fmt}
                      </span>
                    ))}
                  </div>
              ...
              /* Results */
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-surface-divider">
                {/* Summary panel */}
                <div className="p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <span className="text-xs text-ink-muted font-mono truncate max-w-[120px]">{fileName}</span>
                  </div>

                  <div className="mb-5">
                    <p className="text-[11px] font-semibold text-accent uppercase tracking-wider mb-2">Extractive</p>
                  ...
                  <div className="mb-5">
                    <p className="text-[11px] font-semibold text-accent/70 uppercase tracking-wider mb-2">Abstractive</p>
                    <p className="text-sm text-ink-body leading-relaxed">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold text-ink-muted uppercase tracking-wider mb-2">Keywords</p>
                    <div className="flex flex-wrap gap-1.5">
                      {['NLP', 'Summarization', 'TextRank', 'LLM', 'RAG', 'ML'].map((kw) => (
                        <span key={kw} className="px-2.5 py-0.5 rounded-full bg-accent-pill text-accent text-[11px] font-medium">
                          {kw}
                        </span>
                      ))}
                    </div>
                ...
                <div className="p-6 flex flex-col">
                  ...
                  <form onSubmit={handleChat} className="flex gap-2">
                    <input
                      ...
                      disabled={isTyping}
                      className="flex-1 px-4 py-2.5 rounded-xl border border-surface-divider bg-surface-card text-sm text-ink placeholder-ink-muted/50 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent/30 transition-all disabled:opacity-50"
                    />
                    <button
                      type="submit"
                      disabled={isTyping || !chatInput.trim()}
                      className="px-4 py-2.5 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent/90 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      Send
                    </button>
                  </form>
